...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    console.log(
      `[Webhook] ${channel} | user=${user.id} | request_id=${request_id}`,
    );
...
async function processPhoto(
  ...
  if (!embedding) {
    ...
    return postCallback(callbackUrl, requestId, [
      notification(
        "No face detected",
        "Couldn't find a face in the photo. Try again.",
      ),
    ]);
  }
  ...
  if (candidates.length > 0) {
    ...
    if (person) {
      console.log(
        `[Photo] Matched ${person.name} (distance=${distance}) for user ${userId}`,
      );
      return postCallback(callbackUrl, requestId, [
        notification(
          `That's ${person.name}!`,
          `You met them on ${formatDate(new Date(person.met_at))}`,
          true,
        ),
      ]);
    }
  ...
  return postCallback(callbackUrl, requestId, [
    notification(
      "New face captured!",
      'Say "name is [name]" to remember this person.',
    ),
  ]);
}
...
app.post("/mcp", async (req: Request, res: Response) => {
  ...
  if (method === "tools/call" && params.name === "handle_dialog") {
    ...
    if (args.person_name)
      return handleSaveName(res, id, userId, args.person_name, location);
    ...
    if (
      /^(?:list|show|who have i met|people i've met|all people)/.test(utterance)
    )
...
function handleSaveName(
  ...
  if (!pending) {
    return mcpReply(
      ...
      [
        notification(
          "No photo found",
          "Take a photo first, then say the name.",
        ),
      ],
    );
  }
  ...
  return mcpReply(res, id, `Got it! I'll remember ${personName}.`, [
    notification(
      `Remembered: ${personName}`,
      `Saved on ${formatDate(new Date())}`,
    ),
    feedItem(
      `Met ${personName}`,
      `You met ${personName} on ${formatDate(new Date())}.`,
    ),
  ]);
}
...
function handleListPeople(res: Response, id: unknown, userId: string) {
  ...
  if (people.length === 0) {
    return mcpReply(res, id, "You haven't saved anyone yet.", [
      notification(
        "No people saved",
        "Take a photo and say the person's name to get started.",
      ),
    ]);
  }
