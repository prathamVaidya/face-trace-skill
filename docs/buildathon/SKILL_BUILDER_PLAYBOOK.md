...
_Copy the `https` URL from ngrok._
...
Send emails or create calendar events using the user's _own_ accounts without handling tokens.
...
_Requires `mail.send` in your manifest's `allowedTools`._
...
_Note: Timezone and Locale are always provided._
